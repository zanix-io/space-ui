...
An optional, curated set of 27 common UI icons (24 from Font Awesome, plus three original Zanix
...
The 27 names are a closed TypeScript union, not a bare `string` — passing an unknown name is a
...
[`docs/styling.md`](./styling.md)); the newest seven (`heart`/`globe`/`users`/`user-check`/`lock`/
`trash`/`bookmark`) were added against a real consumer need instead — see `NOTICE.md`'s own
"Real-world additions" section. No brand/social icon is in this list — see below for why.
...
`index.ts`/`index.preact.ts`. Nothing about the factory is specific to that particular 27-icon set —
...
per `CatalogIconName`, each with its own real `viewBox` (not normalized to a shared value: 12 of the
27 use a narrower box than the most common `0 0 512 512`, `verified`/`clock`/`shield`'s own
`0 0 24 24` included). ~11.9 KB uncompressed, ~3.9 KB gzip (measured directly off the real file —
this doc's own size estimate had drifted stale before this pass; re-measure rather than
increment-by-feel on a future addition). 24 of the 27 symbols are Font Awesome-sourced;
`verified`/`clock`/`shield` are original Zanix artwork — see `NOTICE.md`'s own "Zanix-original
addition" section.
