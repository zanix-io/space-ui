...
// Confirmed empirically (`deno check` on the direct assignment) before reaching for this, not
// assumed. Safe here because the call sites in `render.ts` only ever pass a plain string tag
// (`'svg'`/`'use'`) with a plain props object — exactly what `createElement`'s own generic overload
// already accepts at runtime; the cast only works around the overload SET being unmatchable, not
// around any real behavioral mismatch.
