...
import { Turnstile } from 'components/Turnstile/index.ts'

function mount(element: ReturnType<typeof Turnstile>) {
...
Deno.test('Turnstile: SSR markup is an inert, empty container — no window/document read', () => {
  const html = renderToStaticMarkup(<Turnstile siteKey='site-key' onVerify={() => {}} />)

  assertStringIncludes(html, 'data-space-ui="turnstile"')
  assertEquals(html.includes('<script'), false)
})

Deno.test('Turnstile: mounting with no turnstile global yet never throws', () => {
  const { root, unmount } = mount(<Turnstile siteKey='site-key' onVerify={() => {}} />)

  assertEquals(root.getAttribute('data-space-ui'), 'turnstile')
  unmount()
})

// --- widget render + onVerify ------------------------------------------------------------------

Deno.test('Turnstile: renders the widget once ready, onVerify fires from callback', async () => {
  const mock = installTurnstileMock()
  const onVerify: string[] = []
  const { unmount } = mount(
    <Turnstile siteKey='site-key' onVerify={(token) => onVerify.push(token)} />,
  )
...
Deno.test('Turnstile: onExpire/onError fire from own callbacks, error code forwarded', async () => {
  const mock = installTurnstileMock()
  let expiredCount = 0
...
Deno.test('Turnstile: verifyTrigger calls execute in appearance="execute" mode', async () => {
  const mock = installTurnstileMock()
...
Deno.test('Turnstile: resetTrigger calls reset(widgetId)', async () => {
  const mock = installTurnstileMock()
  function Wrapper({ resetTrigger }: { resetTrigger: number }) {
    return <Turnstile siteKey='site-key' onVerify={() => {}} resetTrigger={resetTrigger} />
  }
  const container = document.createElement('div')
  document.body.appendChild(container)
  const root = createRoot(container)
...
Deno.test('Turnstile: a script load failure also routes to onError', async () => {
  const errors: unknown[] = []
  const { unmount } = mount(
    <Turnstile siteKey='site-key' onVerify={() => {}} onError={(err) => errors.push(err)} />,
  )
  await act(() => Promise.resolve())
...
Deno.test('Turnstile: scriptSrc overrides the default script URL verbatim', () => {
  const customSrc = 'https://proxy.example.com/turnstile-mirror.js'
  const { unmount } = mount(
    <Turnstile siteKey='site-key' onVerify={() => {}} scriptSrc={customSrc} />,
  )
